...
async def get_users(event):
    ...
    if not sender.id == me.id:
        clown = await event.reply("`processing...`")
    else:
        clown = await edit_or_reply(event, "`processing...`")
    geezteam = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await clown.edit("`Sorry, Can add users here`")
    s = 0
    f = 0
    error = "None"

    await clown.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(geezteam.full_chat.id):
        try:
            if error.startswith("Too"):
                return await clown.edit(
