...
async def _(clown):
    await clown.reply(random.choice(absen))


@register(incoming=True, from_users=DEVS, pattern=r"^.clown$")
async def _(clown):
    await clown.reply(random.choice(pacar))


@clown_cmd(pattern="sping$")
async def redis(pong):
    ...
    await xx.edit(
        ...
        f"**➾Kecepatan : ** '%sms'  \n"
        f"**➾Branch : ** 'Clown-Userbot` \n" % (duration)
    )


@clown_cmd(pattern="lping$")
async def redis(pong):
    """For .ping command, ping the userbot from any chat."""
...
async def pingme(pong):
    ...
    user = await pong.client.get_me()
    await pong.edit(f"**🤡ClownName : [{user.first_name}](tg://user?id={user.id}) **\n📗 `%sms`" % (duration))


@clown_cmd(pattern="pink$")
async def redis(pong):
    """For .ping command, ping the userbot from any chat."""
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
